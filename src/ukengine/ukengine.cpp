...
bool isValidCV(ConSeq c, VowelSeq v)
{
    ...
    if ((c == cs_gi && vInfo.v[0] == vnl_i) || (c == cs_qu && vInfo.v[0] == vnl_u)) {
        return false;
    } // gi doesn't go with i, qu doesn't go with u

    if (c == cs_k) {
        // k can only go with the following vowel sequences
        static VowelSeq kVseq[] = {
            vs_e, vs_i, vs_y, vs_er, vs_eo, vs_eu, vs_eru, vs_ia, vs_ie, vs_ier, vs_ieu, vs_ieru, vs_nil
        };
        int i;
    ...
    switch (ev.evType) {
        ...
        case vneHook_o:
            if (v[1] == vnl_o || v[1] == vnl_or) {
                if (vEnd == m_current && VSeqList[vs].len == 2 && m_buffer[m_current].form == vnw_cv && m_buffer[m_current - 2].cseq == cs_th) {
                    // o|o^ -> o+
                    newVs = VSeqList[vs].withHook;
                    markChange(vStart + 1);
        ...
        default:  //vneHookAll, vneHookUO:
            if (v[0] == vnl_u) {
                if (v[1] == vnl_o || v[1] == vnl_or) {
                    ...
                    if ((vs == vs_uo || vs == vs_uor) && vEnd == m_current && m_buffer[m_current].form == vnw_cv && (m_buffer[m_current - 2].cseq == cs_th || m_buffer[m_current - 2].cseq == cs_h)) {
                        newVs = vs_uoh;
                        markChange(vStart + 1);
                        m_buffer[vStart + 1].vnSym = vnl_oh;
                    } else {
...
int UkEngine::processHook(UkKeyEvent &ev)
{
    ...
    if (VSeqList[vs].len > 1 && ev.evType != vneBowl && (v[0] == vnl_u || v[0] == vnl_uh) && (v[1] == vnl_o || v[1] == vnl_oh || v[1] == vnl_or)) {
...
int UkEngine::getTonePosition(VowelSeq vs, bool terminated)
{
    ...
    if (m_pCtrl->options.modernStyle && (vs == vs_oa || vs == vs_oe || vs == vs_uy)) {
        return 1;
    }
...
int UkEngine::processTone(UkKeyEvent &ev)
{
    ...
    if (m_buffer[m_current].form == vnw_c && (m_buffer[m_current].cseq == cs_gi || m_buffer[m_current].cseq == cs_gin)) {
    ...
    if (m_buffer[m_current].form == vnw_vc || m_buffer[m_current].form == vnw_cvc) {
        ...
        if ((cs == cs_c || cs == cs_ch || cs == cs_p || cs == cs_t) && (ev.tone == 2 || ev.tone == 3 || ev.tone == 4)) {
            return processAppend(ev);
        } // c, ch, p, t suffixes don't allow ` ? ~
    }
...
int UkEngine::processDd(UkKeyEvent &ev)
{
    ...
    if (m_buffer[m_current].form == vnw_nonVn && m_buffer[m_current].vnSym == vnl_d && (m_buffer[m_current - 1].vnSym == vnl_nonVnChar || !IsVnVowel[m_buffer[m_current - 1].vnSym])) {
        m_singleMode = true;
        pos = m_current;
        markChange(pos);
...
int UkEngine::processMapChar(UkKeyEvent &ev)
{
    ...
    if (m_current >= 0 && m_buffer[m_current].form != vnw_empty && m_buffer[m_current].form != vnw_nonVn) {
        return 1;
    }
    ...
    if (entry.form != vnw_empty && entry.form != vnw_nonVn) {
        ...
        if (prevSym == ev.vnSym) {
            if (entry.form != vnw_c) {
                ...
                if (tone != 0 && m_current >= 0 && (m_buffer[m_current].form == vnw_v || m_buffer[m_current].form == vnw_cv)) {
...
int UkEngine::checkEscapeVIQR(UkKeyEvent &ev)
{
    ...
    if (entry.form == vnw_v || entry.form == vnw_cv) {
        ...
        switch (ev.keyCode) {
            ...
            case '~':
            case '.':
                escape = (ch == 'A' || ch == 'E' || ch == 'I' || ch == 'O' || ch == 'U' || ch == 'Y');
                break;
        }
    }
    ...
    switch (ev.chType) {
        ...
        case ukcVn: {
            if (IsVnVowel[ev.vnSym]) {
                ...
                if (m_current >= 0 && m_buffer[m_current].form == vnw_c && ((m_buffer[m_current].cseq == cs_q && v == vnl_u) || (m_buffer[m_current].cseq == cs_g && v == vnl_i))) {
...
int UkEngine::appendVowel(UkKeyEvent &ev)
{
    ...
    if (m_current == 0 || !m_pCtrl->vietKey) {
        ...
        if (!m_pCtrl->vietKey || ((m_pCtrl->charsetId != CONV_CHARSET_UNI_CSTRING) && isalpha(entry.keyCode))) {
            return 0;
        }
        markChange(m_current);
        return 1;
    ...
    if (!autoCompleted && (m_pCtrl->charsetId != CONV_CHARSET_UNI_CSTRING) && isalpha(entry.keyCode)) {
        return 0;
    }
...
{
    if (m_pCtrl->vietKey && m_current >= 0 && m_buffer[m_current].form != vnw_empty && m_buffer[m_current].form != vnw_nonVn) {
        m_toEscape = true;
    }
    return processAppend(ev);
}
...
int UkEngine::processNoSpellCheck(UkKeyEvent &ev)
{
    ...
    if (ev.evType == vneNormal && ((entry.keyCode >= 'a' && entry.keyCode <= 'z') || (entry.keyCode >= 'A' && entry.keyCode <= 'Z'))) {
        return 0;
    }
    markChange(m_current);
    return 1;
...
int UkEngine::process(unsigned int keyCode, int &backs, unsigned char *outBuf, int &outSize, UkOutputType &outType)
{
    ...
    if (m_pCtrl->vietKey && m_current >= 0 && m_buffer[m_current].form == vnw_nonVn && ev.chType == ukcVn && (!m_pCtrl->options.spellCheckEnabled || m_singleMode)) {
...
int UkEngine::getSeqSteps(int first, int last)
{
    ...
    if (m_pCtrl->charsetId == CONV_CHARSET_XUTF8 || m_pCtrl->charsetId == CONV_CHARSET_UNICODE) {
        return (last - first + 1);
    }
...
int UkEngine::processBackspace(int &backs, unsigned char *outBuf, int &outSize, UkOutputType &outType)
{
    ...
    if (m_current == 0 || m_buffer[m_current].form == vnw_empty || m_buffer[m_current].form == vnw_nonVn || m_buffer[m_current].form == vnw_c || m_buffer[m_current - 1].form == vnw_c || m_buffer[m_current - 1].form == vnw_cvc || m_buffer[m_current - 1].form == vnw_vc) {
...
enum VnCaseType
{
    VnCaseNoChange, VnCaseAllCapital, VnCaseAllSmall
};
...
int UkEngine::macroMatch(UkKeyEvent &ev)
{
    ...
    VnConvert(CONV_CHARSET_VNSTANDARD, m_pCtrl->charsetId, (UKBYTE *)macroText, (UKBYTE *)m_pOutBuf, &inLen, &maxOutSize);
    outSize = maxOutSize;
...
bool UkEngine::lastWordIsNonVn()
{
    ...
    switch (m_buffer[m_current].form) {
        ...
        case vnw_cvc: {
            ...
            if ((cs == cs_c || cs == cs_ch || cs == cs_p || cs == cs_t) && (tone == 2 || tone == 3 || tone == 4)) {
                return true;
            }
        }
    }
