...
void drawText(Window win, GC gc, XFontStruct *fontInfo, unsigned int winWith, unsigned int winHeight);

void drawMainWindow(Window win, GC gc, XFontStruct *fontInfo, unsigned int winWidth, unsigned int winHeight);
...
int main(int argc, char **argv)
{
    ...
    MainWindow = XCreateSimpleWindow(display, DefaultRootWindow(display), MainWinX, MainWinY, MainWinWidth, MainWinHeight, 0, WhitePixel(display, ScreenNum), WhitePixel(display, ScreenNum));
    ...
    XSetWMProperties(display, MainWindow, &windowNameP, &iconNameP, argv, argc, sizeHints, wmHints, classHints);

    XChangeProperty(display, MainWindow, _NET_WM_WINDOW_TYPE, XA_ATOM, 32, PropModeReplace, (unsigned char *)&_NET_WM_WINDOW_TYPE_DOCK, 1);

    //  XStoreName(display, im_window, "Unikey");
    XSetTransientForHint(display, MainWindow, MainWindow);
    XSelectInput(display, MainWindow, StructureNotifyMask | ExposureMask | ButtonPressMask | ButtonReleaseMask | VisibilityChangeMask | PointerMotionMask | PropertyChangeMask);
...
void drawMainWindow(Window win, GC gc, XFontStruct *fontInfo, unsigned int winWidth, unsigned int winHeight)
{
    ...
    XDrawString(display, win, gc, (winWidth - textWidth) / 2, (winHeight + fontHeight) / 2 - 2, text, len);
}
