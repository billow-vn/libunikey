...
{
    return SyncTranslate(uk, UkToSyncCharsetList, sizeof(UkToSyncCharsetList) / sizeof(SyncMap), VKC_UTF8);
}

//--------------------------------------------
int SyncToUnikeyCharset(int sync)
{
    return SyncTranslate(sync, SyncToUkCharsetList, sizeof(SyncToUkCharsetList) / sizeof(SyncMap), UNIKEY_UTF8);
}

//--------------------------------------------
int UnikeyToSyncMethod(int uk)
{
    return SyncTranslate(uk, UkToSyncMethodList, sizeof(UkToSyncMethodList) / sizeof(SyncMap), VKM_TELEX);
}

//--------------------------------------------
int SyncToUnikeyMethod(int sync)
{
    return SyncTranslate(sync, SyncToUkMethodList, sizeof(SyncToUkMethodList) / sizeof(SyncMap), UNIKEY_TELEX_INPUT);
}
...
long atom_get_value(Atom key)
{
    ...
    if (XGetWindowProperty(display, RootWindow, key, 0, //long offset
        1, //long length
        False, //no delete
...
{
    XChangeProperty(display, RootWindow, atom, XA_CARDINAL, 32, PropModeReplace, (unsigned char *)&value, 1);
}
