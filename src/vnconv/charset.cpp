...
int SingleByteCharset::putChar(ByteOutStream &os, StdVnChar stdChar, int &outLen)
{
    ...
    if (stdChar >= VnStdCharOffset) {
        ...
        if (ch == 0) {
            ch = (stdChar == StdStartQuote) ? PadStartQuote : ((stdChar == StdEndQuote) ? PadEndQuote : ((stdChar == StdEllipsis) ? PadEllipsis : PadChar));
        }
        ret = os.putB(ch);
    } else {
...
UnicodeCharset::UnicodeCharset(UnicodeChar *vnChars)
{
    ...
    m_toUnicode = vnChars;
    for (i = 0; i < TOTAL_VNCHARS; i++) {
        m_vnChars[i] = (i << 16) + vnChars[i];
    } // high word is used for index
...
int UnicodeCharset::putChar(ByteOutStream &os, StdVnChar stdChar, int &outLen)
{
    ...
    return os.putW((stdChar >= VnStdCharOffset) ? m_toUnicode[stdChar - VnStdCharOffset] : (UnicodeChar)stdChar);
}
...
int UnicodeCompCharset::nextInput(ByteInStream &is, StdVnChar &stdChar, int &bytesRead)
{
    ...
    UniCompCharInfo *pInfo = (UniCompCharInfo *)bsearch(&key, m_info, m_totalChars, sizeof(UniCompCharInfo), uniCompInfoCompare);
    if (!pInfo) {
        ...
        if (is.peekNextW(w)) {
            ...
            if (hi > 0) {
                key.compChar += hi << 16;
                pInfo = (UniCompCharInfo *)bsearch(&key, m_info, m_totalChars, sizeof(UniCompCharInfo), uniCompInfoCompare);
                if (pInfo) {
                    stdChar = pInfo->stdIndex + VnStdCharOffset;
                    bytesRead += 2;
                    is.getNextW(w);
...
{
    UnicodeChar uChar = (stdChar < VnStdCharOffset) ? (UnicodeChar)stdChar : m_toUnicode[stdChar - VnStdCharOffset];
    int ret;
    if (uChar < 0x0080) {
        outLen = 1;
...
int VIQRCharset::nextInput(ByteInStream &is, StdVnChar &stdChar, int &bytesRead)
{
    ...
    if (stdChar < 256) {
        ...
        if ((!VnCharsetLibObj.m_options.smartViqr || m_atWordBeginning) && upper == 'D' && (ch2 == 'd' || ch2 == 'D')) {
            is.getNext(ch2);
            bytesRead++;
            ...
            int cond;
            if (m_suspicious) {
                cond = IS_VOWEL(ch1) && (index == 2 || index == 4 || index == 8 || //not accepting ? . in suspicious mode
                    (index == 12 && (upper == 'A' || upper == 'E' || upper == 'O')) || (m_stdMap[ch2] == 24 && upper == 'A') || (m_stdMap[ch2] == 26 && (upper == 'O' || upper == 'U')));
                if (cond) {
                    m_suspicious = 0;
                }
            } else {
                cond = IS_VOWEL(ch1) && ((index <= 10 && index > 0 && (!m_gotTone || (index != 6 && index != 10))) || (index == 12 && (upper == 'A' || upper == 'E' || upper == 'O')) || (m_stdMap[ch2] == 24 && upper == 'A') || (m_stdMap[ch2] == 26 && (upper == 'O' || upper == 'U')));
            }

            if (cond) {
                if (index > 0) {
                    m_gotTone = 1;
                } //we have a tone/breve/hook in the current word
...
int VIQRCharset::putChar(ByteOutStream &os, StdVnChar stdChar, int &outLen)
{
    ...
    if (stdChar >= VnStdCharOffset) {
        ...
        if (stdChar > 255) {
            ...
            if (!VnCharsetLibObj.m_options.viqrMixed && !m_noOutEsc && (stdChar == '\\' || (index > 0 && index <= 10 && m_escapeTone) || (index == 12 && m_escapeRoof) || (index == 24 && m_escapeBowl) || (index == 26 && m_escapeHook))) {
...
CVnCharsetLib::~CVnCharsetLib()
{
    ...
    for (i = 0; i < CONV_TOTAL_SINGLE_CHARSETS; i++) {
        if (m_sgCharsets[i]) {
            delete m_sgCharsets[i];
        }
    }

    for (i = 0; i < CONV_TOTAL_DOUBLE_CHARSETS; i++) {
        if (m_dbCharsets[i]) {
            delete m_dbCharsets[i];
        }
    }
...
{
    if (ch >= VnStdCharOffset && ch < (VnStdCharOffset + TOTAL_ALPHA_VNCHARS) && IS_ODD(ch)) {
        ch -= 1;
    }
    return ch;
}
...
{
    if (ch >= VnStdCharOffset && ch < (VnStdCharOffset + TOTAL_ALPHA_VNCHARS) && IS_EVEN(ch)) {
        ch += 1;
    }
    return ch;
}
